...
export default function AddPlan() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/plans', { ...form, duration: Number(form.duration), price: Number(form.price) });
      navigate('/plans');
    } catch (err) {
      ...
      <div className="card max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input name="planName" value={form.planName} onChange={handleChange} className="input-field" required placeholder="Monthly Lunch Plan" />
          </div>
          <div>
